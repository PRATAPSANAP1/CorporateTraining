    ...
    for (let i = 0; i < publicExamples.length; i++) {
      ...
      if (config.judge0ApiKey) {
        try {
          ...
          outputStatus = 'judge0_unavailable'; 
          error = 'Judge0 API is unavailable. Please try again later.';
          allPassed = false;
        }
      } else {
        outputStatus = 'accepted';
...
const submitCode = async (req, res) => {
  try {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      if (config.judge0ApiKey) {
        try {
          ...
          finalStatus = 'judge0_unavailable';
          runError = 'Judge0 API is unavailable. Please try again later.';
          break;
        }
      } else {
        isCasePassed = true;
    ...
    // Update user streak dynamically on code submission
